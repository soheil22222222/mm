...
local function lock_group_all(msg, data, target)
  ...
  if group_all_lock == 'yes' then
    return 'تنظیمات کلی از قبل قفل بود🔒'
  else
...
local function lock_group_leave(msg, data, target)
  ...
  if group_leave_lock == 'yes' then
    return 'قفل لفت دادن از قبل فعال بود🔒'
  else
...
local function unlock_group_leave(msg, data, target)
  ...
  if group_leave_lock == 'no' then
    return 'هشدار قفل لفت فعال نیست🔊'
  else
    data[tostring(target)]['settings']['leave'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل لفت ازاد شد🔓'
  end
end
...
local function lock_group_reply(msg, data, target)
  ...
  if group_reply_lock == 'yes' then
    return 'قفل ریپلی از قبل فعال بود🔒'
  else
...
local function unlock_group_reply(msg, data, target)
  ...
  if group_reply_lock == 'no' then
    return 'هشدارریپلی قفل نشده🔊'
  else
    data[tostring(target)]['settings']['reply'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل ریپلی ازاد شد🔓'
  end
end
...
local function lock_group_username(msg, data, target)
  ...
  if group_username_lock == 'yes' then
    return 'قفل یوزرنیم از قبل فعال بود🔒'
  else
    data[tostring(target)]['settings']['username'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'فرستان یوزرنیم قفل شد🔐'
  end
end
...
local function unlock_group_fosh(msg, data, target)
  ...
  if group_fosh_lock == 'no' then
    return 'هشدار فحش دادن قفل نشده🔊'
  else
...
local function lock_group_emoji(msg, data, target)
  ...
  if group_emoji_lock == 'yes' then
    return 'قفل اموجی از قبل فعال بود🔒'
  else
...
local function unlock_group_emoji(msg, data, target)
  ...
  if group_emoji_lock == 'no' then
    return 'هشدار فرستان اموجی قفل نشده🔊'
  else
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'id' then
			...
			elseif msg.text:match("@[%a%d]") then
				...
				return "👤اسم شما: " ..string.gsub(msg.from.print_name, "_", " ").. "\n\n👤یوزرنیم شما: @"..(msg.from.username or '----').."\n\n👤ایدی شما: "..msg.from.id.."\n\n👥اسم سوپرگروه: " ..string.gsub(msg.to.print_name, "_", " ").. "\n\n👥ایدی سوپرگروه: "..msg.to.id
			end
		end
