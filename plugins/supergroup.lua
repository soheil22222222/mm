...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'add' and not matches[2] then
			...
			print("سوپرگروه "..msg.to.print_name.."("..msg.to.id..") فعال شد")
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] سوپرگروه فعال شد")
			superadd(msg)
			set_mutes(msg.to.id)
		...
		if matches[1] == 'rem' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, 'سوپر گروه اضافه نشده', ok_cb, false)
			end
		...
		if matches[1] == "gpinfo" then
			...
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] مشخصات سوپرگروه")
		...
		if matches[1] == "admins" then
			...
			member_type = 'Admins'
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] لیست ناظمان سوپرگروه")
		...
		if matches[1] == "bots" and is_momod(msg) then
			member_type = 'Bots'
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] لیست بات های سوپرگروه")
		...
		if matches[1] == 'link' then
			...
			if not group_link then
				return "شما میتوانید بادستور /newlink لینک جدید باسازید"
			end
		...
		if matches[1] == 'promote' then
		  if not is_momod(msg) then
			...
			if not is_owner(msg) then
				return "فقط مدیران و صاحبان گروه میتوانند شاخصی راناظر کنند"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'promote',
		...
		if matches[1] == 'demote' then
			...
			if not is_owner(msg) then
				return "فقط مدیران  میتوانند ناظری انتخاب کنند"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'demote',
		...
		if matches[1] == "setusername" and is_admin1(msg) then
			local function ok_username_cb (extra, success, result)
				local receiver = extra.receiver
				if success == 1 then
					send_large_msg(receiver, "یوزرنیم سوپر گروه ذخیره شد")
		...
		if msg.media then
			if msg.media.type == 'photo' and data[tostring(msg.to.id)]['settings']['set_photo'] == 'waiting' and is_momod(msg) then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] عکس جدید سوپر گروه تنظیم شد")
		...
		if matches[1] == 'setphoto' and is_momod(msg) then
			...
			return 'لطفا عکس جدید سوپرگروه را ارسال کنید'
		end
		...
		if matches[1] == 'clean' then
			...
			if matches[2] == 'about' then
				...
				local data_cat = 'description'
				if data[tostring(msg.to.id)][data_cat] == nil then
					return 'توضیحات ثبت نشد'
				end
				data[tostring(msg.to.id)][data_cat] = nil
				save_data(_config.moderation.data, data)
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] cleaned about")
				channel_set_about(receiver, about_text, ok_cb, false)
				return "توضیحات گروه حذف شد"
			end
			if matches[2] == 'silentlist' then
				chat_id = msg.to.id
				local hash =  'mute_user:'..chat_id
					redis:del(hash)
				return "لیست افراد خاموش "
			end
			if matches[2] == 'username' and is_admin1(msg) then
				local function ok_username_cb (extra, success, result)
					...
					elseif success == 0 then
						send_large_msg(receiver, "خطا در یافت یوزرنیم سوپرگروه")
					end
				end
				local username = ""
				channel_set_username(receiver, username, ok_username_cb, {receiver=receiver})
		...
		if matches[1] == 'setflood' then
			...
			return 'حساسیت اسپم تغییرکرد: '..matches[2]
		end
		...
		if matches[1] == 'mute' and is_owner(msg) then
			...
			local msg_type = 'Audio'
				if not is_muted(chat_id, msg_type..': yes') then
					...
					return msg_type.." قفل ویس فعال شد"
				else
					return "سکوت ویس فعال شد "..msg_type.." فعال شد"
				end
			end
			...
			local msg_type = 'Photo'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return msg_type.." قفل عکس فعال شد"
				else
					return "قفل "..msg_type.." is already on"
				end
			end
