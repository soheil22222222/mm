...
local function run(msg, matches)
  if msg.to.type == 'user' then
    ...
    if matches[2] == "setflood" then
      ...
      if tonumber(matches[3]) < 1 or tonumber(matches[3]) > 200 then
        return " تنظیم کنید [1-200]شما میتوانید حساسیت اسپم را از بین "
      end
    ...
    if matches[2] == 'get' then 
      if matches[3] == 'link' then
        if not is_owner2(msg.from.id, chat_id) then
          return "این گروه برای ربات نیست"
        end
